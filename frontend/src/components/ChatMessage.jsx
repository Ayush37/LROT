    ...
    if (data.name === 'sync_adjustments') {
	          ...
	          <div className="ml-3">
	            ...
	            <div className="mt-3">
	              <div className="text-xs text-green-700">
	                <p><span className="font-semibold">Adjustment Type:</span> {result.adjustment_type}</p>
	                <p><span className="font-semibold">DMAT IDs Processed:</span> {result.dmat_ids.join(', ')}</p>
	              </div>
	            </div>
	          </div>
	        </div>
	  ...
	  return (
	    <div className="mt-3 bg-gray-50 border border-gray-200 rounded-lg p-4">
	      <p className="text-sm text-gray-600">Unable to process the sync request.</p>
	    </div>
	  );
	}    	
